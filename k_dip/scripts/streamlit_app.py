...
from k_dip.scripts.common import (
    load_models,
    parse_args,
    img_to_html,
    get_page_image,
...
in_file: UploadedFile = st.sidebar.file_uploader(
    ...
    type=["pdf", "png", "jpg", "jpeg", "gif", "pptx", "docx", "xlsx"],
)
...
run_marker = st.sidebar.button("Run OCR")

use_llm = st.sidebar.checkbox(
    "Use LLM", help="Use LLM for higher quality processing", value=False
)
chart_desc = st.sidebar.checkbox(
    "Chart Description",
    help="Generate chart descriptions using LLM for charts",
    value=False,
)
llm_ocr = st.sidebar.checkbox(
    "Use LLM OCR",
    help="Generate chart descriptions using LLM for charts",
    value=False,
)
...
with tempfile.TemporaryDirectory() as tmp_dir:
    ...
    print(f"Option chart_desc: {chart_desc}")
    cli_options.update(
        {
            ...
            "llm_service": "k_dip.services.openai.OpenAIService",
            "api_key": "EMPTY",
